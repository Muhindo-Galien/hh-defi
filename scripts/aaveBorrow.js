...
const { getPushLength } = require("hardhat/internal/hardhat-network/stack-traces/opcodes.js")

async function main() {
    await getWeth()
    const { deployer } = await getNamedAccounts()
    const lendingPool = await getLendingPool(deployer)
    const wethToken =networkConfig[network.config.chainId].wethToken
...
async function getLendingPool(account) {
    const lendingPoolAddressesProvider = await ethers.getContractAt("ILendingPoolAddressesProvider", networkConfig[network.config.chainId].lendingPoolAddressesProvider,
    account)
